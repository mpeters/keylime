...
def read_allowlist(al_path=None, checksum="", gpg_sig_file=None, gpg_key_file=None):
    ...
    if gpg_sig_file and gpg_key_file:
        ...
        with open(gpg_key_file, 'r') as key_f:
            logger.debug(f"Importing GPG key {gpg_key_file}")
        ...
        with open(gpg_sig_file, 'rb') as sig_f:
            logger.debug(f"Comparing allowlist ({al_path}) against GPG signature ({gpg_sig_file})")
            verified = gpg.verify_file(sig_f, al_path)
            if verified:
                logger.debug("Allowlist passed GPG signature verification")
            else:
                raise Exception(f"Allowlist GPG signature verification failed comparing allowlist ({al_path} against {gpg_sig_file}")

    # Purposefully die if path doesn't exist
    with open(al_path, 'rb') as f:
        logger.debug(f"Loading allowlist from {al_path}")
        alist_bytes = f.read()
        sha256 = hashlib.sha256()
        sha256.update(alist_bytes)
        calculated_checksum = sha256.hexdigest()
        alist_raw = alist_bytes.decode("utf-8")
        logger.debug(f"Loaded allowlist from {al_path} with checksum {calculated_checksum}")
    ...
    if p.match(alist_raw):
        ...
        if "meta" in alist and "version" in alist["meta"]:
            ...
            if int(version) == ALLOWLIST_CURRENT_VERSION:
                logger.debug(f"Allowlist has compatible version {version}")
            else:
            ...
            logger.debug(f"Allowlist does not specify a version. Assuming current version {ALLOWLIST_CURRENT_VERSION}")
    else:
